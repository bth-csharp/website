...
```shell title="I terminalen står du i katalogen kmom02/practice och gör följande"
...
:::note
Om det skapas en .sln fil i VS Code av misstag, så ta bort den. Den ställer till det vid kompilering och debuggning och används inte i VS Code.
När vi skapar med dotnet new console i terminalen skapas ingen .sln fil.
:::
...
### Klass med hjälpmetoder - Helpers

#### Beskrivning och klassdiagram - Helpers
...
```shell title="Kodstruktur i katalogen MenuApp2 med klassen Helpers i src"
...
```bash title="Så här ser filen src/Helper.cs med den tomma klassen Helpers ut"
...
```csharp title="Lägg till följande kod i klassen Helpers"
namespace MenuApp2.src;
...
#### Beskrivning och klassdiagram - Menu
...
```shell title="Skapa klassen Menu och se att strukturen i MenuApp2 ser ut så här:"
...
```csharp title="Lägg till följande kod i klassen Men i filen src/Menu.cs"
namespace MenuApp2.src;
...
```csharp title="Huvudprogrammet i Program.cs med modern C# med toplevel statements"
...
```csharp title="Huvudprogrammet i Program.cs som det såg i .NET 5 och tidigare med"
using System;
using MenuApp2.src;
...
```bash title="Ladda ner filen AsciiArt.cs och kopiera den till src katalogen. Stå i MenuApp2 katalogen."
...
```csharp title="Början av klasser AcsiiArt ser ut så här:"
namespace MenuApp2.src;  
...
```csharp title="I klassen Menu anropas metoden PrintDie i klassen AsciiArt"
namespace MenuApp2.src;
...
            switch (choice)
            {
...
```csharp title="Lägg till metoden PrintDateAndTime i klassen Helpers"
...
```csharp title="Klassen Helpers med veckodagarna på svenska"
namespace MenuApp2.src;
...
```csharp title="Skriv ut alla veckodagarna med Join i klassen Helpers"
namespace MenuApp2.src;
...
public class Helpers
{
    ...
    {
        // Skriv ut som en sträng
        Console.WriteLine("\nVeckodagarna på svenska: (sträng)");
...
```csharp title="I klassen Helpers loopar vi igenom arrayen sweWeekdays med foreach"
namespace MenuApp2.src;
...
public class Helpers
{
    ...
    private static void PrintDays()
    {
        ...
        // Skriv ut arrayen med foreach
        Console.WriteLine("\nVeckodagarna på svenska: (array)");
...
```csharp title="I klassen Helpers gör vi om arrayen sweWeekdays till en lista och skriver ut den med foreach"
namespace MenuApp2.src;
...
<Figure
    src={MenuApp2_4}
    caption="Så här ser det ut i terminalen med menyalternativ 4."
    width="500px"
/>
