...
De olika kontotyperna har olika aktiviteter i slutet av månaden men själva aktiviteten är ska alla konton göra. Då kan vi skapa en metod, `PerformMonthEndTransactions()`, i basklassen som vi sedan överskuggas (override) i subklasserna. För att visa att vi vill att en metod ska överskuggas så markerar vi den **virtual** i basklassen och **override** i subklasserna. Om vi vill lägga till en tredje subklass och glömmer att implementera metoden `PerformMonthEndTransactions()` i den, så får vi kompileringsfel när metoden är **virtual** i basklassen.
