...
Vi överskuggar metoden `Description` med specifik information om subklassen. Även i denna metod kan vi lägga till raden `base.Description();` om vi vill ha med utskrifterna från basklassernas `Description` metoder.

### Testa subklassen **ElSuv**

Vi skapar ett objekt av subklassen **ElSuv** med märket "Ford Capri" som har en räckvidd på 627 km och  en laddningstid på ca 28 minuter.
...
När vi anropar metoden `Description` på variabeln "elSuv" så är det metoden på subklassen **ElSuv** som anropas. Vi kan spara samma objekt av subklassen **ElSuv** i variabler av typen **Car** och **Suv** och få samma resultat.
...
Metoden `Description` på klassen **Suv** anropas då.
...
Console.WriteLine($"Testar Equals() på objektet car2 och car: {car2.Equals(car)}.");
// ger utskriften: Testar Equals() på objektet car2 och car: False.
```
...
Nu ser det ut som innan med den skillnaden att vi har ett objekt av klassen **ElSuv** och dess ASCII-bild istället för ett objekt av klassen **Suv**.
...
Vi använder `base.` för att komma åt metoder och attribut på basklassen. Åtkomstnivån på dessa måste vara åtminstone protected för att subklassen ska komma åt dem.
