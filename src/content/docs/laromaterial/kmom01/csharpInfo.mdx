...
import csharpKompDetaljer from '@assets/img/kmom01/csharpKompDetaljer.png';
...
Kompilering av ett C#-program sker följande i steg:

1. Källkod på datorn
Du skriver och sparar din C# kod lokalt på din dator. Filer: `*.cs`

1. C#-kompilering (Roslyn)
När du kör *dotnet build* kompileras C#-koden (`*.cs-filer`) först till CIL(Common Intermediate Language), som är plattformsoberoende.
Denna CIL sparas i en `.dll-fil` för klassbibliotek eller `.exe-fil` för körbar kod (assembly).
Vid C#-kompileringen kollas syntax och datatyper kontrolleras.

1. Körning under .NET/CLR (Common Language Runtime)
När du sedan kör programmet med *dotnet run*:
  1. CLR (Common Language Runtime) laddar CIL-koden.
  1. Verifiering (säkerhet och typkorrekthet)
  1. JIT-kompilatorn översätter CIL → maskinkod för din dator precis innan metoderna körs.
  Därför: “Just in time”.
  1. Exekvering av ditt program


```text
.cs (C#-källkod)
   └─(C#-kompilator)→  .dll/.exe (CIL + metadata)
                           └─(CLR + JIT vid körning)→  native maskinkod i minne
                                                          └→ exekveras av CPU
```

En fördel med JIT-kompilering är att koden optimeras för aktuell CPU (dator) och operativsystem men nackdelen är att det tar lite längre tid att starta ett C#-program än ett Python-program.  
En annan fördel är portabilitet då samma CIL kan köras på olika plattformar/CPU:er; JIT skräddarsyr maskinkoden.
...
.NET är en kraftfull plattform som möjliggör effektiv och flexibel programutveckling genom att erbjuda ett brett utbud av verktyg, språk och ramverk som kan anpassas för att möta olika utvecklingsbehov och krav.
...
- **dotnet build**; bygger ett .NET-projekt och dess beroenden, med andra ord kompilerar koden.
...
När du deklarerar variabler så behöver du inte allokera plats i minnet för det sköter C# om automatiskt. Likaså rensas minnet automatiskt då det kan finnas oanvända objekt som inte kan nås, så kallad automatisk skräpinsamling. Det är denna automatiska skräpinsamling som gör att C# inte lämpar sig för realtidsapplikationer.
...
- värdetyper;
  ...
  - nullvärden
  - tuplar, för att gruppera flera dataelement i en enkel struktur
- referenstyper;
  - som string och array (blir objekt)
  - innehåller en referens till ett objekt i minnet
  - du kan definiera egna
