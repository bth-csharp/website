...
För att debugga vår kod öppnar vi vårt projekt, det vill säga katalogen med vårt program *FirstApp*, i VS Code. Markera filen "Program.cs". Först klickar du på "Run" eller "Run and Debug", sen på "Add configuration" och sen välj "C#". Nu kan du markera filen "Program.cs" och köra igång debuggern.

Efter som "C# Dev Kit extension" är installerad, så kan du debugga genom att markera filen "Program.cs" och sen klicka på "F5". Det funkar inte på min Mac utan där gör jag enligt bilden nedan. Klicka först (1) på "SOLUTION EXPLORER" och därefter (2) så markerar du "FirstApp". Högerklicka på "FirstApp" och följande meny dyker upp med (3) debug som alternativ längst ner. Starta (4) debuggern genom att välja "Start New Instance".

Se bild nedan:

<Figure
    src={debug1}
    caption="Starta debuggning i VS Code."
/>
...
Sätt brytpunkter genom att markerar till vänster om radnumret och då blir det en röd punkt där.
