...
I C# finns dessutom operatorerna `++` som ökar värdet med 1 och `--` som minska värdet med 1.
...
I C#, liksom i Python, finns också en `do while`-loop och den kan se ut så här:
...
Python har ingen support för array (fält på svenska), istället använder vi List för att skapa arrayer i Python. En array i Python kan innehålla olika datatyper:
...
Ett fält (array på engelska) i C# innehåller element av samma datatyp:
...
Istället för att skriva samma kodrader på flera ställen så vill vi placera dessa i funktioner/metoder. I C# kallar används benämningen metoder och dessa innehåller kodrader som utför en specifik uppgift.
...
- var kan användas när typen på en variabel istället bestäms av initieringen
- array (fält) finns
