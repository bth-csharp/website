...
Microsoft/.NET-teamet tillhandahåller nyare versioner av .NET för Ubuntu via ett särskilt arkiv (PPA = Personal Package Archive), utöver de versioner som finns i Ubuntus vanliga paketförråd. Backports-paketets arkiv (ppa:dotnet/backports) innehåller nyare versioner av .NET SDK och .NET Runtime.

```shell
sudo add-apt-repository ppa:dotnet/backports
```

Med .NET SDK (SDK = Software Developers Kit) kan du utveckla appar med .NET. Om du installerar .NET SDK behöver du inte installera motsvarande runtime. Kör följande kommandon för att installera .NET SDK:
