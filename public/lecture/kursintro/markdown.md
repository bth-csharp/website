# Kursen C#


**Introduktion till kursen**


Marie Grahn




## Agenda
<hr style="color:green">

* Kursformalia
* Innehåll
* Labbmiljö




## Hitta kursmaterialet
<hr style="color:green">

* Gå in på [canvas.bth.se](https://canvas.bth.se) och klicka på länkar
* Gå till [bth-csharp.github.io/website/](https://bth-csharp.github.io/website/)





## Översikt av kursen
<hr style="color:green">

* [bth-csharp.github.io/website/intro](https://bth-csharp.github.io/website/intro)

Läs igenom artiklarna under "introduktion" så har du koll. Det är din innehållsförteckning av kursen.




## Kursnamn och kod
<hr style="color:green">

* Objektorienterad programmering i CSharp
* DV1701
* 7.5hp
* Datavetenskap/Programvaruteknik




## Kursplan
<hr style="color:green">

* Kursplanen är ett formellt styrdokument till kursen som bla beskriver detaljer, förkunskaper, syfte, innehåll, mål och litteratur.

* Låt oss kika på en del information som går att läsa i kursplanen.

<p class="footnote">Kursplan: <a href="https://plan.bth.se/courses/DV1701">https://plan.bth.se/courses/DV1701</a></p>




## Förkunskaper

    Genomgången kurs i programmering om 7,5 hp.


## Innehåll
<hr style="color:green">

* Grundläggande programmering i C# specifik syntax, som variabler, metoder, datatyper, operatorer, in- och utmatning i terminal, arrayer, listor, villkor, loopar och switchsatser samt funktioner
* Objektorienterade programmeringstekniker, som klasser, objekt, inkapsling, överlagring, överskuggning och klassrelationer (exempelvis arv och komposition)


## Innehåll...
<hr style="color:green">

* Introduktion till objektorienterad analys och design med UML
* Mer avancerad objektorientering, som polymorfism, abstrakta klasser och metoder samt interface
* Specifika koncept och funktioner inom C#, som filstruktur (namespace), in- och utmatning till filer, felhantering och undantag
* Testning med enhetstester
* Utvecklingsmiljö och verktyg för utveckling av C# inklusive verktyg för kodkvalitet


## Mål kursplan
<hr style="color:green">

* Kunskap och förståelse

    "Redogöra för grundläggande kunskaper om objektorienterad programmering, som klasser, objekt, inkapsling, överlagring, överskuggning och klassrelationer (exempelvis arv och komposition)."


## Mål kursplan...
<hr style="color:green">

* Kunskap och förståelse...

    "Tillämpa mer avancerad objektorienterad programmering, som polymorfism, abstrakta klasser och metoder samt interface."

    "Påvisa grundläggande kunskaper i att använda UML och enhetstestning."





## Mål kursplan...
<hr style="color:green">

* Färdighet och förmåga

    "Självständigt kunna utveckla, dokumentera och presentera ett program i objektorienterad C# utifrån en specifikation."

    "Analysera, designa och förklara en objektorienterad lösning med UML."

    "Ha god praktisk förmåga att hantera verktyg och utvecklingsmiljöer som används vid utveckling med C#."




## Mål kursplan...
<hr style="color:green">

* Värderingsförmåga och förhållningssätt

    "Visa förmåga att kritiskt granska sin kod och identifiera förbättringsområden."


## Kursliteratur
<hr style="color:green">

* Bok: C# 12 in a Nutshell: The Definitive Reference - Joseph Albahari
* Material från institutionen.
* Resurser via nätet

<p class="footnote">Läsanvisningar finns i varje kmom.</p>




## Planering
<hr style="color:green">

* [https://bth-csharp.github.io/website/intro/studieplan/](https://bth-csharp.github.io/website/intro/studieplan/)
    * Rekommenderad studieplan
    * BTH Schema




## Läromiljö
<hr style="color:green">

* [Canvas](https://bth.instructure.com/)
* [https://bth-csharp.github.io/website/](https://bth-csharp.github.io/website/)
* [Kursrepo GitHub](https://github.com/orgs/bth-csharp/repositories)
* Discord
* Zoom
* [YouTube](https://www.youtube.com/???)




## Intressenter
<hr style="color:green">

* Studenter
* Lärarteamet
* Programansvariga
* Institutionen för Datavetenskap
    * Prefekt
    * Studierektor
* Utbildningsföreningen BITS




## Examination
<hr style="color:green">

* Varje kmom lämnas in för kontroll (en eller flera uppgifter)
    * U, Ux, PG, G
* Individuell examination med projektet och muntligt förhör
    * A-F, Fx




## Ladok
<hr style="color:green">

| Moment | Beskrivning | Poäng | Betyg |
|--------|-------------|-------|-------|
|Kmom01-03|Inlämningsuppgift 1|2,5hp|GU|
|Kmom04-06|Inlämningsuppgift 2|2,5hp|GU|
|Kmom10|Individuellt projekt|2,5hp|GU|


<p class="footnote">Kursens slutbetyg är enligt FA när samtliga moment är avklarade.</p>




## Kursvärdering & kursutveckling
<hr style="color:green">

- BTH kursvärdering
- Samla feedback på GitHub issue
- Feedback i redovisningstexter
- Avslutande kommentar om kursen med betyg på kursen




## Dialog, samarbete & fusk
<hr style="color:green">

- Diskutera med studiekollegor kring problem och lösningar på de övningar och uppgifter som finns
- Tveka inte att starta egna studiegrupper där ni kan stödja varandra.
- **Kopiera inte kod av varandra**
- [Fusk, AI och medvetet vilseledande](https://bth-csharp.github.io/website/studieguide/fusk/)
<p class="footnote">Allt misstänkt fusk anmäls.</p>





## Kursmoment
<hr style="color:green">

* 01: Kom igång med C#
* 02: Introduktion till Objektorienterad programmering och UML
* 03: Mer Objektorienterad programmering och UML
* 04: Repetition C#, OOP & UML samt kodintervju
* 05: Arv & mer testning
* 06: Begrepp inom OOP & Kodkvalitet
* 07/10: Projekt och examinationer




## Ambitioner
<hr style="color:green">

- Kunna utveckla objektorienterade program i C#
- Lära dig objektorienterad analys och design med UML
- Testning med enhetstester
- Lär dig verktyg, miljöer och felsökning


## Farhågor
<hr style="color:green">

- Nybörjare: Mycket nytt på en gång
- Känsla: Det är mycket nytt på en gång och det är nya saker varje vecka.
- Önskan: Jag vill ha mer tid att lära mig konstruktioner.
- Utmaning: Förstå konstruktionerna och använda rätt saker på rätt sätt vid rätt tillfälle.




## Labbmiljö
<hr style="color:green">

- Windows, Mac, Linux
- Text editor, Visual Studio Code
- Terminal Bash
- Kursrepo GitHub
- C#
- .NET
- NUnit för enhetstestning


## Varför C#?

<hr style="color:green">

* Modernt och lättläst språk
* Objektorienterat
* Kompilerande språk som är starkt typat och säkert
* Kraftfullt tillsammans med .NET
* Plattformsoberoende – fungerar på Windows, macOS och Linux.
* Används i industrin, t. ex. inom spelutveckling (Unity) och webbtjänster

<h2 style="color:green">Slut</h2>

Börja jobba nu?

Installera labbmiljön, skapa kursrepot och sätt upp utvecklingsmiljön.

Lycka till!
